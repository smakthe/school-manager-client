...
export function SchoolTabs({
  ...
  const fetchClassrooms = async () => {
    let all: any[] = [],
      ...
      total = 1;
    do {
      const res = await classroomsApi.list(page, school.id);
      all = [...all, ...res.data];
  ...
  const fetchTeachers = async () => {
    let all: any[] = [],
      ...
      total = 1;
    do {
      const res = await teachersApi.list(page, school.id);
      all = [...all, ...res.data];
      total = res.meta.pages || 1;
      page++;
    } while (page <= total);
  ...
  const fetchStudents = async (page = 1) => {
    ...
    try {
      const res = await studentsApi.list(page, school.id);
  ...
  return (
    <div className="space-y-4">
      <Button variant="ghost" onClick={onBack} className="-ml-4 mb-2">
